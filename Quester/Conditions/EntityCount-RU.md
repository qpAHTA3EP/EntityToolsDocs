...
|<a name ="ref-Sign">***Sign***</a> | Переключатель, который задает соотношение референтного значения [*Value*](!ref-Value) с подсчитанным количеством *Entity*:<br/>- ***Equal*** : значение свойства должно быть РАВНО [*Value*](!ref-Value);<br/>- ***NotEqual*** : значение свойства должно быть НЕ РАВНО [*Value*](!ref-Value) с точностью до 1.0;<br/>- ***Inferior*** : значение свойства должно быть МЕНЬШЕ [*Value*](!ref-Value);<br/>- ***Superior*** : значение свойства должно быть БОЛЬШЕ [*Value*](!ref-Value).
