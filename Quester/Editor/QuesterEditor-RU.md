...
3) Возможность редактирования отдельного quester-профиля, не связанного с активным quester-профилем, управляющим действиями персонажа.  
   ...
   Следствием данного подхода является то, что внесенные в редакторе изменения не отражаются на поведение персонажа в игре. Чтобы это произошло требуется [**выгрузить** редактируемый ***quester-профиль*** в исполнительный движок бота (*Engine*)](ProfileToolsPanel-RU.md#ref-UploadProfile).   
...
На данной панели инструментов сгруппированы кнопки управления файлом профиля. [*Подробнее...*](ProfileToolsPanel-RU.md)
...
На этой панели отображается список точек патрулирования, используемых для некоторых команд. Например, *Grind*, *InteractNode*, [*MoveToEntity*](../Actions/MoveToEntity-EN.md) и т.п. [*Подробнее...*](HotSpostPanel-RU.md)  
